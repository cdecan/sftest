import { HEALTH_CRITICAL_HP, HEALTH_DAMAGE_COLOR, HEALTH_MAX_HP, KO_ANIMATION, KO_FLASH_DELAY, TIME_DELAY, TIME_FLASH_DELAY, TIME_FRAME_KEYS } from "../../constants/battle.js";
...
export class StatusBar {
    ...
    drawHealthBars(context){
        ...
        context.fillRect(
            32, 21,
            HEALTH_MAX_HP - Math.floor(this.healthBars[0].hitPoints), 9,
        );
        context.fillRect(
            208 + Math.floor(this.healthBars[1].hitPoints), 21,
            HEALTH_MAX_HP - Math.floor(this.healthBars[1].hitPoints), 9,
        );
    }
    ...
    updateKoIcon(time){
        if(this.healthBars.every((healthBar) => healthBar.hitPoints > HEALTH_CRITICAL_HP)) return;
    ...
    updateHealthBars(time){
        for(const index in this.healthBars){
            ...
            this.healthBars[index].hitPoints = Math.max(0, this.healthBars[index].hitPoints - (time.delta * FPS));
        }
    }
